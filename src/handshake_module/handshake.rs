...
impl Handshake {
    pub fn new(private_key: SecretKey, remote_public_key: PublicKey) -> Self {
        Handshake {
            ecies: EllipticCurveIES::new(private_key, remote_public_key),
            secrets: None,
        }
    }
    ...
    pub fn encrypt(&self, data_in: BytesMut, data_out: &mut BytesMut) -> Result<usize> {
        self.ecies.encrypt_data(data_in, data_out)
    }
    ...
    pub fn decrypt<'a>(
        ...
        self.ecies.decrypt_data(data_in, read_bytes)
    }
