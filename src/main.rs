...
pub mod handshake_module;
pub mod hash_module;
pub mod utils_module;
pub mod constants_module;

use crate::{
    utils_module::utils::{Error, Result, Disconnect, Hello},
    handshake_module::handshake::Handshake,
};
...
async fn main() {
    ...
    if let Ok(mut stream) = TcpStream::connect(node_address).await {
        ...
        if let Err(e) = initiate_handshake(&mut stream, node_public_key).await {
            println!("{e}");
        }
    } else {
...
async fn initiate_handshake(stream: &mut TcpStream, node_public_key: PublicKey) -> Result<()> {
    ...
    println!("Sent auth message to target node 👋");
